...
if (isset($_GET['id'])) {
 ...
 if ($_SERVER["REQUEST_METHOD"] === "POST") {
  try {
	// SQL query to update tag name
    $sql = "UPDATE `taq` SET `taq_name`=? WHERE `id` =?";
    $taq_name = $_POST['taq_name'];
    $stmt = $conn->prepare($sql);
    $stmt->execute([$taq_name, $id]);
  ...
  try {
	// Selecting tag data based on provided 'id'
      $sql = "SELECT * FROM `taq` WHERE `id` = ?";
      $stmt = $conn->prepare($sql);
      $stmt->execute([$id]);
      $result = $stmt->fetch();
	  $taq_name = $result['taq_name'];
									...
									<form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" action="" method="post">
										...
										<div class="item form-group">
											...
											<div class="col-md-6 col-sm-6 ">
												<input type="text" id="add-category" required="required" class="form-control " name="taq_name" value="<?php echo $taq_name ?>">
											</div>
										</div>
